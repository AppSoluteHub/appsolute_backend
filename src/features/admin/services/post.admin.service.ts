...
import { PostAdminData, PostData, UpdatePostData } from "../../../interfaces/post.interface";
import {
  AppError,
  BadRequestError,
  ForbiddenError,
...
class PostService {
  static async createPost(userId: string, postData: PostAdminData) {
    ...
    if (!Array.isArray(category) || category.length === 0) {
        throw new BadRequestError("Category must be a non-empty array");
      }
      
      // Sanitize each category string
      const postCategory = category.map(cat => cat.trim());
